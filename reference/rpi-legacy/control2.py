...
def get_profile_torque(phase, gain):
    """
    支持跨零(Wrapping)的非对称力矩生成:
    允许 T_ONSET > T_OFFSET (例如 0.9 -> 0.3)
    """
   
    # --- 1. 计算"相对进度" (归一化到从 Onset 开始) ---
    # 我们把 T_ONSET 当作新的 0 点，计算当前 phase 跑了多远
   
    # 计算当前相位距离起点的距离 (处理循环)
    progress_from_onset = phase - T_ONSET
    if progress_from_onset < 0:
        progress_from_onset += 1.0
       
    # 计算整个助力窗口的总长度 (处理循环)
    total_window_width = T_OFFSET - T_ONSET
    if total_window_width < 0:
        total_window_width += 1.0
       
    # 计算峰值点距离起点的距离 (处理循环)
    peak_location_from_onset = T_PEAK - T_ONSET
    if peak_location_from_onset < 0:
        peak_location_from_onset += 1.0

    # --- 2. 范围检查 ---
    # 如果跑出的距离超过了窗口总长，说明不在助力区
    if progress_from_onset > total_window_width:
        return 0.0
   
    # --- 3. 上升段 (Rise) ---
    if progress_from_onset < peak_location_from_onset:
        duration = peak_location_from_onset
        if duration <= 1e-6: return PEAK_TORQUE * gain
       
        # 归一化进度 0~1
        p = progress_from_onset / duration
        # 半余弦插值 (0->1)
    ...
    # --- 4. 下降段 (Fall) ---
        ...
        duration = total_window_width - peak_location_from_onset
        if duration <= 1e-6: return 0.0
       
        # 归一化进度 0~1
        p = (progress_from_onset - peak_location_from_onset) / duration
        # 半余弦插值 (1->0)
        scale = 0.5 * (1 + math.cos(math.pi * p))
        return scale * PEAK_TORQUE * gain
